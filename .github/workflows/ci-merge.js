...
const MERGE_LABEL = (process.env.MERGE_LABEL) || 'nightly-merge';
...
async function checkPRChanges(github, context) {
...
async function tagAndPush(github, owner, repo, execa, commit=false) {
    ...
    let tagsList = tags.repository.refs.nodes;
    if (tagsList.length < 1)
        throw Error(`No previous tags found for ${owner}/${repo}.`);
...
async function mergePullRequests(pulls, execa) {
    ...
    await execa("git", ["config", "--global", "user.name", "lmmsbot"]);
    await execa("git", [
        "config",
        "--global",
        "user.email",
        "lmms\x2eservice\x40gmail\x2ecom", // prevent email harvesters from scraping the address
    ]);
    let hasFailed = false;
    for (let pull of pulls) {
        let pr = pull.number;
...
async function mergebot(github, context, execa) {
    ...
    await fetchPullRequests(pulls, "https://github.com/LMMS/lmms", execa);
    const mergeResults = await mergePullRequests(pulls, execa);
    await generateReadme(pulls, context, mergeResults, execa);
    await tagAndPush(github, context.repo.owner, `${context.repo.repo}-nightly`, execa, true);
}

module.exports.mergebot = mergebot;
module.exports.checkPRChanges = checkPRChanges;
